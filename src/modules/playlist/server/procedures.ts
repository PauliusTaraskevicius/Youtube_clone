...
export const playlistsRouter = createTRPCRouter({
  remove: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const { id } = input;
      const { id: userdId } = ctx.user;

      const [deletedPlaylist] = await db
        .delete(playlists)
        .where(and(eq(playlists.id, id), eq(playlists.userId, userdId)))
        .returning();
  ...
  getVideos: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const [existingPlaylist] = await db
        ...
        .from(playlists)
        .where(
          and(eq(playlists.id, playlistId), eq(playlists.userId, userdId))
        );
      ...
      const viewerVideoReactions = db.$with("viewer_video_reactions").as(
        db
          ...
          .where(
            and(
              eq(videoReactions.type, "like"),
              eq(videoReactions.userId, userdId)
            )
          )
      );
    ...
    .query(async ({ input, ctx }) => {
      ...
      const viewerVideoViews = db.$with("viewer_video_views").as(
        db
          ...
          .from(videoViews)
          .where(eq(videoViews.userId, userdId))
      );
